...
def replace_node(tg: _TransportGraph, node_id: int, new_attrs):
    ...
    reset_descendents(tg, node_id)
    print(f"Invalidated descendents of node {node_id}")


def reset_node(tg: _TransportGraph, node_id: int):
...
def reset_descendents(tg, node_id):
    try:
...
def apply_electron_updates(tg: _TransportGraph, electron_updates: dict):
    for n in tg._graph.nodes:
        ...
        if name in electron_updates:
            replace_node(tg, n, electron_updates[name])
    return tg
