...
# Until dispatches are submitted using asyncio tasks, the sublattices
# will be actually dispatched, thus will use a separate process for execution.
# This is because we don't want to deserialize the sublattice inside the dispatcher,
# and we are accessing its result from the common dictionary `futures` in `entry_point.py`.
# Either that or we build the graph of sublattice when we did it for lattice on client side.
...
def graceful_result_failure(dispatch_id: str, exception: Exception):
    """
    Gracefully mark the result object as failed and store the exception and
    other relevant information

    Args:
        dispatch_id: Dispatch ID of the result to be marked as failed
...
def run_workflow(dispatch_id: str, json_lattice: str) -> Result:
    """
    ...
    try:
        ...
        result_object = asyncio.run(_run_planned_workflow(result_object))

    except Exception as ex:
        graceful_result_failure(dispatch_id, ex)
        raise
