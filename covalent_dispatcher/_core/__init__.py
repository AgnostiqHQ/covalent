...
def get_task_inputs(task_input: dict, node_id: int, node_name: str, result_object: Result) -> dict:
    """
...
def post_process(lattice: Lattice, node_outputs: dict) -> Any:
    """
...
def _run_planned_workflow(result_object: Result) -> Result:
    ...
    for nodes in order:
        ...
        for node_id in nodes:
            ...
            task_input = get_task_inputs(task_input, node_id, node_name, result_object)
    ...
    result_object._result = post_process(result_object.lattice, node_outputs_for_post_processing)
            ...
            workflow_schedule["nodes"].append(
                {
                    ...
                    "backend_args": {"results_dir": node["metadata"]["results_dir"]},
                }
            )
