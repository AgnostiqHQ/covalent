...
@click.option(
    "--covalent_services",
    is_flag=True,
    help="Use post covalent_services cli command [with Supervisord]",
)
@click.option(
    "-p",
    "--port",
...
def start(covalent_services, port: int, develop: bool) -> None:
    if covalent_services:
        _sd_start_services()
    else:
...
def status(covalent_services) -> None:
    if covalent_services:
        _sd_status()
    else:
        if _read_pid(UI_PIDFILE) != -1:
...
def stop(covalent_services) -> None:
    if covalent_services:
        _sd_stop_services()
    else:
...
@click.pass_context
def restart(ctx, port: int, develop: bool, covalent_services: bool) -> None:
    """
...
@click.pass_context
def status(ctx, port: int, develop: bool, covalent_services: bool) -> None:
    """
