...
from refactor.queuer.app.core.api import APIService
...
class TestAPIService:
    ...
    mocked_json = {"test": "dict"}

    mocked_params = {"test": "params"}

    mocked_data = {"test": "data"}

    mocked_files = {"filename": b""}
    ...
    def test_delete(self, request_delete_mock, api_service_format_mock):
        ...
        api_service_format_mock.assert_called()
        request_delete_mock.assert_called_once_with(
            self.mocked_endpoint, params=self.mocked_params
        )
    ...
    def test_put(self, request_put_mock, api_service_format_mock):
        ...
        api_service_format_mock.assert_called()
        request_put_mock.assert_called_once_with(
            self.mocked_endpoint,
            json=self.mocked_json,
            params=self.mocked_params,
            data=self.mocked_data,
