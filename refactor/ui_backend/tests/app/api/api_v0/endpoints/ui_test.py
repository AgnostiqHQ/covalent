...
async def test_throttle_request_update(test_app, monkeypatch):
    ...
    for i in range(0, 25):
        tasks.append(
            asyncio.create_task(
                throttle_request_update_notify(test_app, MOCK_DISPATCH_ID, MOCK_TASK_ID)
            )
        )
...
def test_ui_update_endpoint(test_app, mocker):
    ...
    response = test_app.put(
        f"/api/v0/ui/workflow/{MOCK_DISPATCH_ID}/task/{MOCK_TASK_ID}",
    )
...
async def test_draft_endpoint(test_app, mocker):
    ...
    response = test_app.post("/api/v0/ui/workflow/draft", json=REQUEST_BODY)
    assert response.status_code == 200
