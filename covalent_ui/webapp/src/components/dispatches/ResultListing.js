...
const ResultListing = () => {
  ...
  const dashboardListViewInitial = useSelector(
  ...
  const selectResultsCache = (state) => state.dashboard.dashboardList
  ...
  const dashboardListView = useSelector((state) =>
    !selectNormQuery(state, searchKey)
      ? dashboardListViewInitial
      : selectSearchResults(state, searchKey).map((e) => {
        return {
          dispatchId: e.item.dispatch_id,
          endTime: e.item.ended_at,
          latticeName: e.item.lattice_name,
          resultsDir: e.item.results_dir,
          status: e.item.status,
          error: e.item.error,
          runTime: e.item.runtime,
          startTime: e.item.started_at,
          totalElectrons: e.item.total_electrons,
          totalElectronsCompleted: e.item.total_electrons_completed,
        }
      })
  )
  const onSearch = (e) => {
