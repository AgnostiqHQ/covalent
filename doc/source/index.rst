...
.. image:: _static/covalent_platform.png
   :align: center
...
The most recent stable release of Covalent comes with several new enhancements:

- Dask is integrated into core Covalent as the default executor (compute backend) for workflows. This provides a finer control over task resources in the short term while a leaner custom solution is developed.
- We have updated the Covalent CLI to allow users the flexibility to choose the default compute backend. This is injected into workflows and tasks at runtime if no other backend is specified.
- We have made it simpler for users to parse logs generated by Covalent using the command `covalent logs`.
