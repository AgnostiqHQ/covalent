...
def result_2():
    ...
    result = Result(
        lattice=received_lattice, results_dir=TEMP_RESULTS_DIR, dispatch_id="dispatch_2"
    )
    result.lattice.metadata["results_dir"] = TEMP_RESULTS_DIR
    result._initialize_nodes()
    return result
...
def test_result_persist_workflow_1(test_db, result_1, mocker):
    ...
    with test_db.session() as session:
        ...
        job_rows = session.query(Job).all()

        assert len(electron_rows) == len(job_rows)
...
def test_result_persist_subworkflow_1(test_db, result_1, result_2, mocker):
    ...
    # Mark parent electron's job as cancelled
    with test_db.session() as session:
    ...
    with test_db.session() as session:
        ...
        # Check that cancel request is propagated to sublattice jobs
